...
button.addEventListener("click", (e) => {
    ...
    console.log(titre, annee, type);


    let apiUrl = `http://www.omdbapi.com/?apikey=${key}&S=${titre}&Type=${type}&y=${annee}`;
        ...
        .then((response) => {
            (response.json())
                ...
                .then((data) => {
                    ...
                    afficherCard(data)
                    //card.innerHTML = `<p> ${data.Title} : ${data.Year} </p>`
                })
                .catch((error) => {
                    console.error(error)
